...
type RootStackParamList = {
  ...
  ProfileEntrepriseVisite: { entrepriseId: number };
};
interface Post {
  id: number;
  title: string;
...
const ProfileEntrepriseVisite = () => {
 ...
 const [entreprise, setEntreprise] = useState<Entreprise | null>(null);
   const [publicites, setPublicites] = useState<PubliciteImage[]>([]);
     ...
     const route = useRoute<RouteProp<RootStackParamList, 'ProfileEntrepriseVisite'>>(); // Corrigez ici
     const { entrepriseId } = route.params;
      const [posts, setPosts] = useState<Post[]>([]);
       const [events, setEvents] = useState<Event[]>([]);
         const [activities, setActivities] = useState<Activity[]>([]);
        const filteredEvents = events.filter(e => e.entreprise_id === entrepriseId);
const filteredPosts = posts.filter(p => p.entreprise_id === entrepriseId);
     useEffect(() => {
      if (!entrepriseId) {
        console.error('Entreprise ID is not available');
        return;
      }
    
      const fetchEntrepriseDetails = async () => {
        try {
          const response = await authApi.get(`/entreprises/${entrepriseId}`);
          console.log('Détails de l\'entreprise:', response.data); // Vérifiez les données ici
      ...
      const fetchActivities = async (entrepriseId: number) => {
        try {
          const response = await api.get(`/entreprises/${entrepriseId}/activities`);
...
const fetchPosts = async (entrepriseId: number) => {
  try {
    const postsResponse = await api.get(`/entreprises/${entrepriseId}/posts`);
    console.log('Réponse brute posts:', postsResponse.data);
    if (Array.isArray(postsResponse.data)) {
      setPosts(postsResponse.data);
      console.log('Posts utilisés (array):', postsResponse.data);
    } else if (postsResponse.data && postsResponse.data.postes) {
      setPosts(postsResponse.data.postes);
      console.log('Posts utilisés (postes):', postsResponse.data.postes);
    } else {
      setPosts([]);
      console.error('Les données des publications ne sont pas valides:', postsResponse.data);
    }
  } catch (error) {
    setPosts([]);
...
const fetchEvents = async (entrepriseId: number) => {
  try {
    const eventsResponse = await api.get(`/entreprises/${entrepriseId}/events`);
    console.log('Réponse brute events:', eventsResponse.data);
    if (Array.isArray(eventsResponse.data)) {
      setEvents(eventsResponse.data);
      console.log('Events utilisés (array):', eventsResponse.data);
    } else if (eventsResponse.data && eventsResponse.data.events) {
      setEvents(eventsResponse.data.events);
      console.log('Events utilisés (events):', eventsResponse.data.events);
    } else {
      setEvents([]);
      console.error('Les données des événements ne sont pas valides:', eventsResponse.data);
    }
  } catch (error) {
    setEvents([]);
    console.error('Erreur lors de la récupération des événements:', error);
      ...
      fetchActivities(entrepriseId);
      fetchPosts(entrepriseId);
      fetchEvents(entrepriseId);
      fetchPublicites();
...
const getFullImageUrl = (path: string) => {
  ...
  />
      ...
      <View style={styles.activityHeader}>
        <Text style={[styles.sectionTitle, isDarkMode && styles.sectionTitleDark]}>
          {language === 'English' ? 'Welcom' : language === 'French' ? 'Bonjour' : language === 'Arabic' ? 'نشاط' : 'Bonjour'}
          <Text style={styles.companyName}>  {entreprise?.nomE || 'Nom de l\'entreprise'}</Text>
  ...
  {entreprise?.lien_facebook_E && (
    <TouchableOpacity onPress={() => openLink(entreprise.lien_facebook_E)} style={styles.linkContainer}>
      <Text style={styles.linkText}>Facebook</Text>
    </TouchableOpacity>
  )}
  {entreprise?.lien_site_E && (
    <TouchableOpacity onPress={() => openLink(entreprise.lien_site_E)} style={styles.linkContainer}>
  ...
  {publicites.length > 0 ? (
    <>
      <Image
        ...
        resizeMode="cover"
        onError={(e) => {
          console.log('Erreur de chargement de l\'image de publicité:', e.nativeEvent.error);
        }}
...
<View style={styles.gridContainer}>
  {activities.length > 0 ? (
      ...
      <View key={rowIndex} style={styles.gridRow}>
        {row.map(activity => (
          <TouchableOpacity
            ...
            onPress={() => navigation.navigate('ActivityDetailScreen', { activityId: activity.idActP })}
          >
            <Image
              source={
    ...
    <Text style={styles.noActivitiesText}>Aucune activité disponible</Text>
  )}
</View>
          {/* Événements */}
      // ...existing code...
...
{filteredEvents.length > 0 ? (
  chunkArray(filteredEvents, 3).map((row, rowIndex) => (
    <View key={rowIndex} style={styles.gridRow}>
      {row.map(event => (
        <TouchableOpacity
          key={event.id}
...
{filteredPosts.length > 0 ? (
  chunkArray(filteredPosts, 3).map((row, rowIndex) => (
    <View key={rowIndex} style={styles.gridRow}>
      {row.map(post => (
        <TouchableOpacity
          key={post.id}
          ...
          <Image
            source={
              post.images && post.images.length > 0 && post.images[0].url
                ? { uri: getFullImageUrl(post.images[0].url) }
                : require('../assets/placeholder.png')
            }
            style={styles.gridImage}
          />
...
export default ProfileEntrepriseVisite;