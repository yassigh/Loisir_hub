...
const NewPassword = () => {
  const navigation = useNavigation<NewPasswordScreenNavigationProp>(); // 🔥 Typage correct
  
  const route = useRoute();
  const routeParams = route.params as { email?: string; resetCode?: string };
  const email = routeParams?.email ?? 'email_non_reçu';
...
console.log('Reset Code reçu dans NewPassword:', routeParams?.resetCode);
  ...
  const handleSavePassword = async () => {
    ...
    if (password !== confirmPassword) {
      ...
      return;
    }console.log('Email:', email);  // Ajoutez cette ligne pour vérifier que l'email est correct

 setIsLoading(true); 
    try {
      const response = await axios.post('http://192.168.100.122:8001/api/user/reset-password', {
        email,
      ...
      if (response.data) {
        ...
        navigation.navigate('LoginScreen'); // Rediriger vers la page de connexion
      } else {
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Form Content */}
      <Text style={styles.mainTitle}>Setup New Password</Text>
